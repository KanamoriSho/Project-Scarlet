...
public class ShotMove : MonoBehaviour
{
    ...
    private EnemyCharactorMove _charactorMove = default;    //敵キャラクターの挙動スクリプトの格納用
    ...
    private void Reset()
    {
        ...
        if (_shotMoveData._shooterType != ShotMoveData.ShooterType.Player)
        {
            ...
            if (!_isVelocityChangePerShot)
            {
                ...
                //_charactorMoveから現在の生成弾数を受け取る
                _pelletCounter = _charactorMove.GetCurrentPelletCount;

                //_charactorMoveから同時生成弾数を受け取る
